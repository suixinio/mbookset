...
func (b *Blog) Find(blogId int) (*Blog, error) {
	...
	err := o.QueryTable(b.TableName()).Filter("blog_id", blogId).One(b)
	if err != nil {
...
func (b *Blog) FindByIdAndMemberId(blogId, memberId int) (*Blog, error) {
	...
	err := o.QueryTable(b.TableName()).Filter("blog_id", blogId).Filter("member_id", memberId).One(b)
	if err != nil {
		beego.Error("查询文章时失败 -> ", err)
		return nil, err
	}
...
func (b *Blog) FindByIdentify(identify string) (*Blog, error) {
	...
	err := o.QueryTable(b.TableName()).Filter("blog_identify", identify).One(b)
	if err != nil {
		beego.Error("查询文章时失败 -> ", err)
		return nil, err
	}
...
func (b *Blog) Link() (*Blog, error) {
	...
	if b.BlogType == 1 && b.DocumentId > 0 {
		doc := NewDocument()
		if err := o.QueryTable(doc.TableName()).Filter("document_id", b.DocumentId).One(doc, "release", "markdown", "identify", "book_id"); err != nil {
			...
			book := NewBook()
			if err := o.QueryTable(book.TableName()).Filter("book_id", doc.BookId).One(book, "identify"); err != nil {
	...
	if b.ModifyAt > 0 {
		member := NewMember()
		if err := o.QueryTable(member.TableName()).Filter("member_id", b.ModifyAt).One(member, "real_name", "account"); err == nil {
			if member.Nickname != "" {
				b.ModifyRealName = member.Nickname
			} else {
	...
	if b.MemberId > 0 {
		member := NewMember()
		if err := o.QueryTable(member.TableName()).Filter("member_id", b.MemberId).One(member, "nickname", "account", "avatar"); err == nil {
			if member.Nickname != "" {
				b.CreateName = member.Nickname
			} else {
...
//	o := orm.NewOrm()
//
//	return o.QueryTable(b.TableName()).Filter("blog_identify", identify).Exist()
//}
//
////保存文章
...
//	o := orm.NewOrm()
//
//	_, err := o.QueryTable(b.TableName()).Filter("blog_id", blogId).Delete()
//	if err != nil {
...
func (b *Blog) QueryNext(blogId int) (*Blog, error) {
	...
	if err := o.QueryTable(b.TableName()).Filter("blog_id", blogId).One(blog, "order_index"); err != nil {
		beego.Error("查询文章时出错 ->", err)
		return b, err
	}

	err := o.QueryTable(b.TableName()).Filter("order_index__gte", blog.OrderIndex).Filter("blog_id__gt", blogId).OrderBy("order_index", "blog_id").One(blog)
...
func (b *Blog) QueryPrevious(blogId int) (*Blog, error) {
	...
	err := o.QueryTable(b.TableName()).Filter("order_index__lte", blog.OrderIndex).Filter("blog_id__lt", blogId).OrderBy("-order_index", "-blog_id").One(blog)
...
func (b *Blog) LinkAttach() (err error) {
	...
	//当不是关联文章时，用文章ID去查询附件
	if b.BlogType != 1 || b.DocumentId <= 0 {
		_, err = o.QueryTable(NewAttachment().TableName()).Filter("document_id", b.BlogId).Filter("book_id", 0).All(&attachList)
		if err != nil && err != orm.ErrNoRows {
			beego.Error("查询文章附件时出错 ->", err)
		}
	} else {
		_, err = o.QueryTable(NewAttachment().TableName()).Filter("document_id", b.DocumentId).Filter("book_id", b.BookId).All(&attachList)
